...
class CartItem implements Arrayable, Jsonable, \JsonSerializable
{
    ...
    /**
     * The shipping fee for the cart item.
     *
     * @var int|float
     */
     ...
     * @param array      $options
     */
    public function __construct($id, $name, $image, $price, $weight = 0, array $options = [])
    {
        if (empty($id)) {
    ...
    public static function fromArray(array $attributes)
    {
        ...
        return new self($attributes['id'], $attributes['name'], $attributes['image'], $attributes['price'], $attributes['weight'], $options);
    }
     ...
     * @return \Gloudemans\Shoppingcart\CartItem
     */
    public static function fromAttributes($id, $name, $image, $price, $weight, array $options = [])
    {
        return new self($id, $name, $image, $price, $weight, $options);
    }
    ...
    public function toArray()
    {
        return [
            ...
            'id'       => $this->id,
            'name'     => $this->name,
            'slug'     => ($this->associatedModel) ? $this->__get('model')->slug : '',
            'image'    => $this->image,
            'qty'      => $this->qty,
            'price'    => $this->price(),
            'weight'   => $this->weight,
    ...
    //Added By Me (With Implemets \JsonSerializable)
    public function jsonSerialize() {
        return Array(
            ...
            'shipping'      => $this->shipping(),
            'subtotal'      => $this->subtotal(),
            'priceFloat'    => PriceConverterHelper::ConvertPrice($this->price, false),
        );
    }
...
//        return MyCompanySetting::first()->refCurrency->symbol . number_format($value, $decimals, $decimalPoint, $thousandSeperator);
