...
use App\Helpers\PriceConverterHelper;
use App\MyCompanySetting;
use Carbon\Carbon;
use Closure;
...
class Cart
{
     ...
     */
    public function add($id, $name = null, $image = null, $qty = null, $price = null, $weight = 0, array $options = [])
    {
        if ($this->isMulti($id)) {
        ...
        $cartItem = $this->createCartItem($id, $name, $image, $qty, $price, $weight, $options);
    ...
    {
        return (($this->getContent()->count() > 0) ? $this->deliveryFee : 0) + $this->getContent()->reduce(function ($total, CartItem $cartItem) {
    ...
    /**
     * Get the total delivery fees of the items in the cart.
     *
     * @return float
     */
    public function deliveryFloat()
    {
        return (($this->getContent()->count() > 0) ? $this->deliveryFee : 0);
    }

    /**
     * Get the total delivery fees of the items in the cart as formatted string.
     *
     * @param int    $decimals
     * @param string $decimalPoint
     * @param string $thousandSeperator
     ...
     * @return string
     */
    public function delivery($decimals = null, $decimalPoint = null, $thousandSeperator = null)
    {
        return $this->numberFormat($this->deliveryFloat(), $decimals, $decimalPoint, $thousandSeperator);
    }

    /**
     * Get the subtotal (total - tax - delivery) of the items in the cart.
     *
     * @return float
     */
    ...
    /**
     * Get the subtotal (total - tax - delivery) of the items in the cart as formatted string.
     *
     * @param int    $decimals
     * @param string $decimalPoint
     * @param string $thousandSeperator
    ...
    /**
     * Set the global delivery fee for the cart.
     * This will set the tax rate for all items.
     *
     * @param float $discount
     */
    public function setDeliveryFee($delivery)
    {
...
//        $this->getConnection()->table($this->getTableName())->where('identifier', $identifier)->delete();
     ...
     * @return \Gloudemans\Shoppingcart\CartItem
     */
    private function createCartItem($id, $name, $image, $qty, $price, $weight, array $options)
    {
        if ($id instanceof Buyable) {
            ...
            $cartItem = CartItem::fromAttributes($id, $name, $image, $price, $weight, $options);
...
//        return ($value == 0) ? '' : MyCompanySetting::first()->refCurrency->symbol . number_format($value, $decimals, $decimalPoint, $thousandSeperator);
        return ($value == 0) ? '' : PriceConverterHelper::ConvertPrice($value);
    }
